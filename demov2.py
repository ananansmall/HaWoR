...
Core pipeline identical to demo.py, with additional:
  - OpenCV rendering (vis_cam, vis_world, vis_verify)
  - reconstruction npz saving
  - Data file copying to output directory

Usage:
  python demov2.py --video_path example/video_0.mp4
  python demov2.py --video_path example/video_0.mp4 --img_focal 600
"""
...
def generate_vis_verify(seq_folder, img_focal, start_idx, end_idx,
    ...
    for hand_id in detected_hands:
        ...
        R_c2w_np = R_c2w_slice.numpy() if hasattr(R_c2w_slice, 'numpy') else np.array(R_c2w_slice)
        R_w2c_np = R_c2w_np.transpose(0, 2, 1)

        t_c2w_slice = t_c2w_sla_all[:num_total]
        t_c2w_np = t_c2w_slice.numpy() if hasattr(t_c2w_slice, 'numpy') else np.array(t_c2w_slice)
...
if __name__ == '__main__':
    ...
    # ================================================================
    # Core pipeline: identical to demo.py
    # ================================================================
    ...
    # ================================================================
    # MANO forward + R_x transform: identical to demo.py
    # ================================================================
    ...
    # ================================================================
    # Additional: output directory, visualization, data saving
    # ================================================================
    ...
    print(f"Frames: {start_idx} to {end_idx}, hands: {detected_hands}")
    print(f"Image: {img_w}x{img_h}, focal={img_focal:.1f}, cx={cx:.1f}, cy={cy:.1f}, scale={slam_scale:.4f}")
    ...
    print(f"Generating vis_cam ({num_frames} frames)...")
    for i in range(num_frames):
    ...
    print(f"  vis_cam saved to: {cam_output_pth}/")

    # --- vis_world ---
    print(f"Generating vis_world...")
    ...
    np.savez(save_path,
             ...
             start_idx=start_idx,
             end_idx=end_idx)
    print(f"  reconstruction npz saved to: {save_path}")

    # --- Copy data files ---
    print(f"Copying data files to output directory...")
    data_items = [
    ...
    print(f"\nAll output saved to: {output_base}")
    print("finish")
